...
#include <boost/unordered_set.hpp>	//MTM
#include <sstream>
#include <cmath>
...
private:
    ...
    typedef boost::unordered_map<Symbol, Probability>         SymbolToProbMap;	//MTM
    typedef boost::unordered_map<PCFGRule, Probability, PCFGRule::Hasher>       RuleToProbMap;	//MTM
...
//        std::cout << "EMTrainer: Completed after " << no_of_loops << " iterations with a RMSE = " << last_changes << ".";
...
//        std::cout << "EMTrainer: Completed " << iterations << " iterations until RMSE was " << last_changes << " (<= " << threshold << ").";
    }
...
//        std::cout << "EMTrainer: Estimate probabilities for " << no_of_sentences << " sentences.";
...
//                std::cout << "EMTrainer: Current sentence: '" << symbol_vector_to_string(cit->first) << "'";
...
//                std::cout << "EMTrainer: Inside Probability for the whole sentence is " << inside_sentence;
...
//                	std::cout << "EMTrainer: Skipping sentence because of 0-probability.";
                }
            }
        }
...
//        	std::cout << "EMTrainer: Maximize the probabilities of all rules in the grammar.";
            for (ProbabilisticContextFreeGrammar::iterator rule = grammar.begin(); rule != grammar.end(); ++rule) {
...
//                std::cout << "EMTrainer: Updating probability for rule '" << *rule << "'. New: " << new_prob;
...
//        	std::cout << "EMTrainer: No estimation or maximization step performed. Please check, if the sentences in the training data can be parsed with the given grammar.";
        }


//        VLOG(2) << "EMTrainer: Root mean square error compared to the last iteration: " << std::sqrt(rmsq_sum/rmsq_n);
//        std::cout << "EMTrainer: Root mean square error compared to the last iteration: " << std::sqrt(rmsq_sum/rmsq_n);
...
//        std::cout << "EMTrainer: Estimation for the symbol '" << signature.resolve_id(symbol) << "' is "<<score;
        return score;
    }
...
//        std::cout << "EMTrainer: Estimation for the rule '" << rule << "': " << score;
        return score;
    }
...
//        std::cout << "EMTrainer: Reading in the training corpus...";
        while (corpus.good()) {
            std::getline(corpus, line);
            if (!line.empty()) {
//                VLOG(6) << "EMTrainer: Reading in line " << line_no << ": '" << line << "'.";
//            	std::cout << "EMTrainer: Reading in line " << line_no << ": '" << line << "'.";
                // Tokenize line
...
//                    	std::cout << "EMTrainer: Sentence in line " << line_no << " will be ignored, the token '" << word<< "' cannot be resolved.";
                        valid = false;
                    }
