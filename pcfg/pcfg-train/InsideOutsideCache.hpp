...
//    typedef std::unordered_map<CachedItem, InsideOutsideProbability>  CacheMap;
    typedef std::map<CachedItem, InsideOutsideProbability>  CacheMap;
...
//        std::cout << "InsideOutsideCache: Converting <" << symbol << "," << (int)begin << "," << (int)end << "> to 64bit key: " << (std::bitset<64>) buffer;
        return buffer;
