...
private:
    ...
    typedef boost::unordered_map<Symbol, RulePointerVector>      SymbolToRuleVectorMap;	//MTM
    typedef boost::unordered_map<Symbol, unsigned>               SymbolToCounterMap;	//MTM
    typedef boost::unordered_map<Symbol, Probability>            SymbolToProbabilityMap;	//MTM
...
//    	std::cout << "PCFG: Cleaning - Starting cleaning process...";
//        VLOG(5) << "PCFG: Cleaning - Currently there are " << productions.size() << " rules in this grammar.";
//    	std::cout << "PCFG: Cleaning - Currently there are " << productions.size() << " rules in this grammar.";
...
//        std::cout << "PCFG: Cleaning - Rebuilding the rule index...";
        build_rule_index();
//        VLOG(5) << "PCFG: Cleaning - Finished rebuilding the rule index!";
//        std::cout << "PCFG: Cleaning - Finished rebuilding the rule index!";

//        VLOG(5) << "PCFG: Cleaning - Rebuilding rhs vectors...";
//        std::cout << "PCFG: Cleaning - Rebuilding rhs vectors...";
        build_rule_rhs_index();
//        VLOG(5) << "PCFG: Cleaning - Finished rebuilding rhs vectors!";
//        std::cout << "PCFG: Cleaning - Finished rebuilding rhs vectors!";
...
//        std::cout << "PCFG: Cleaning - Finished cleaning process! " << no_rules_before_clean - productions.size() << " rules have been deleted!";
...
//            	std::cout << "PCFG: Probabilities for the symbol '" << get_signature().resolve_id(nt) << "' sum up to '" << current_probability << "' and are therefore illegal. Belonging rules will be normalized.";
                for (iterator rule = rules_for_nt.first; rule != rules_for_nt.second; ++rule) {
...
//                    std::cout << "PCFG: Setting '" << line << "' as startsymbol.";
...
//                    	std::cout << "PCFG: Rule in line " << line_no << " is ignored.";
                    }
                }
...
//        	std::cout << "PCFG: The start symbol could not be set, because it is illegal. "
//        	                    "It must be contained in the signature of the grammar.";
        }
